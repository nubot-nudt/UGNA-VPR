...
import numpy as np
import math
from script.utils.utils import plot_features, save_image_saliancy
from script.dm.direct_pose_model import fix_coord_supp
from script.dm.pose_model import vis_pose
...
def render_virtual_imgs(args, pose_perturb, img_idxs, hwf, device, render_kwargs_test, world_setup_dict, scene):
    ''' render nerfw imgs, save unscaled pose and results'''
    H, W, focal = hwf
    rgb_list = []
    rgbs = []
    # 创建保存文件夹
    save_folder="dataset/test_pose"
    ...
    for batch_idx in range(pose_perturb.shape[0]):
        ...
        filename = os.path.join(save_folder, '{:05d}.png'.format(batch_idx))
        imageio.imwrite(filename, rgb8_f)
         #保存位姿
        pose_filename = os.path.join(save_folder, "pose_{:05d}.txt".format(batch_idx))
         # 将中心化的坐标系转回原来的坐标系
        #pose_avg_stats_file = osp.join(args.data_dir, scene) + '/pose_avg_stats.txt'
        pose = pose.cpu().numpy()
        pose_avg_stats_file = args.datadir + '/pose_avg_stats.txt'
        pose_t = inv_concered(pose, pose_avg_stats_file,batch_idx)
        np.savetxt(pose_filename, pose_t)
    rgbs = torch.stack(rgb_list).detach()
    return rgbs

def inv_concered(pose,pose_avg_stats_file,batch_idx):
    E_pose = np.eye(4)
    E_pose[:3] = pose
...
def perturb_single_render_pose(poses, x, angle):
    ...
    for i in range(N_views):
        ...
        #trans_rand = np.random.uniform(-x,x,3) # random number of 3 axis pose perturbation  z轴单独扰动
        # print("trans_rand", trans_rand)
        trans_rand_xy = np.random.uniform(-x, x, 2)
        # 生成第三个元素的随机扰动，从-0.15到0.15之间
        trans_rand_z = np.random.uniform(-0.15, 0.15, 1)
        # 合并成一个长度为3的数组
