...
def fix_coord(args, poses, i_split, camera_params, pose_avg_stats_file='', rescale_coord=True):
    ''' fix coord for 7 Scenes to align with llff style dataset '''

    # get all poses (train+val)
    train_poses = poses[i_split[0]]

    val_poses = poses[i_split[1]]

    test_poses = poses[i_split[2]]
    ...
    all_poses = all_poses[:, :4, :4]

    bounds = np.array([0, 10])  # manual tuned

    if rescale_coord:
        sc = camera_params.pose_scale  # manual tuned factor, align with colmap scale
        all_poses[:, :3, 3] *= sc

        ### quite ugly ###
        # move center of camera pose
        if camera_params.move_all_cam_vec != [0., 0., 0.]:
            all_poses[:, :3, 3] += camera_params.move_all_cam_vec

        if camera_params.pose_scale2 != 1.0:
            all_poses[:, :3, 3] *= camera_params.pose_scale2
        # end of new mod1
...
def render_init(H, W, K, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           near=0., far=1.,
...
def render_path_init(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    H, W, focal = hwf
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, _ = render_init(H, W, K, chunk=chunk, c2w=c2w[:3, :4], **render_kwargs)
...
def create_nerf_init(args,scenes):
    ...
    ##########################
    if args.Datasetname == "Cambridge" :
        if (scenes == "GreatCourt"):
            nerfh_name = "court_nerf"
            print("court_nerf")
        elif (scenes == "KingsCollege"):
            nerfh_name = "kings_nerf"
            print("kings_nerf")
        elif (scenes == "OldHospital"):
            nerfh_name = "hospital_nerf"
            print("hospital_nerf")
        elif (scenes == "ShopFacade"):
            nerfh_name = "shop_nerf"
            print("shop_nerf")
        elif (scenes == "StMarysChurch"):
            nerfh_name = "church_nerf"
            print("church_nerf")
        else:
            print("ERROR!!!!")

    if args.Datasetname == "NEU" :
        nerfh_name = scenes
    if args.Datasetname == "SIASUN" :
        nerfh_name = scenes
    # Load checkpoints
    if args.ft_path is not None and args.ft_path != 'None':
        ckpts = [args.ft_path]
    else:
        ckpts = [os.path.join(basedir, expname, nerfh_name,f) for f in sorted(os.listdir(os.path.join(basedir, expname, nerfh_name))) if
                 'tar' in f]
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split, camera_params = load_blender_data_Cam(args.datadir, args.half_res,
                                                                                         args.testskip)
        print(i_split)
        ...
        poses, bounds = fix_coord(args, poses, i_split, camera_params, pose_avg_stats_file)
        print("poses")
        print(poses.shape)
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = create_nerf_init(args)
    global_step = start
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path_init(render_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=images,
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path_init(render_poses, hwf, K, args.chunk, render_kwargs_test)
            ...
            #     with torch.no_grad():
            #         rgbs_still, _ = render_path_init(render_poses, hwf, args.chunk, render_kwargs_test)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            with torch.no_grad():
                render_path_init(torch.Tensor(poses[i_test]).to(device), hwf, K, args.chunk, render_kwargs_test,
